...
class SFPlaygroundInit:

    def __init__(self):
        SILICONFLOW_ACCESS_TOKEN = get_conf("SILICONFLOW_ACCESS_TOKEN")
        self.client = OpenAI(base_url="https://api.siliconflow.cn/v1/", api_key=SILICONFLOW_ACCESS_TOKEN,
    ...
    def __conversation_message_payload(self, inputs: str, llm_kwargs: dict, history: list, system_prompt: str):
        ...
        self.model = llm_kwargs['llm_model'].replace("SF:", "").strip()
...
if __name__ == '__main__':
    SFP = SFPlaygroundInit()
    r = SFP.generate_chat('你好', {'llm_model': 'Qwen/Qwen2.5-7B-Instruct'}, [], '你是WPSAi')
    for i in r:
        print(i)