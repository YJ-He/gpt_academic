...
# 借鉴https://github.com/jiangfy-ihep/gpt_academic/tree/feature/groq
"""
    该文件中主要包含三个函数
...
def predict_no_ui_long_connection(inputs: str, llm_kwargs: dict, history: list = [], sys_prompt: str = "",
                                  observe_window: list = None, console_slience: bool = False):
    """
...
def handle_error(inputs, llm_kwargs, chatbot, history, chunk_decoded, error_msg):
    ...
    if "bad_request" in error_msg:
        chatbot[-1] = (
        chatbot[-1][0], "[Local Message] 已经超过了模型的最大上下文或是模型格式错误,请尝试削减单次输入的文本量。")
...
def generate_payload(inputs, llm_kwargs, history, system_prompt, stream):
    ...
    if llm_kwargs['llm_model'] in ['groq-llama3-8b-8192', 'groq-llama3-70b-8192', 'groq-gemma-7b-it', 'groq-llama-3.3-70b-versatile','groq-llama-3.2-90b-vision-preview', 'groq-deepseek-r1-distill-llama-70b']:
        tokens = 8192
    elif llm_kwargs['llm_model'] == 'groq-llama2-70b-4096':
        tokens = 4096
    elif llm_kwargs['llm_model'] == 'groq-llama-3.1-70b-versatile':
        tokens = 32768
    ...
    payload = {
        ...
        "messages": messages,
        "temperature": 0.01 if llm_kwargs['temperature'] == 0 else llm_kwargs['temperature'],
        # groq converts a temperature value of 0 to 1e-8.
        "stream": stream,
        "max_tokens": tokens
    }
    try:
